...
class NewmanConfig{
    ...
    async looprun(root_json_file, series = false, verbose = false){
        ...
        if (series) {
          ...
          let results = []
          Promise.each(run_list, value => reflect(this.runCollectionAsync(value.collection, value.environment)).then(d => results.push(d)))
          .then(() => {
            const success = results.filter(r => r.status === 'fulfilled')
            const failed = results.filter(r => r.status === 'rejected')
            console.log(chalk.green.bold(`Passed (${success.length}):`))
            console.log(success.map(s => s.v.collection))
            console.log(chalk.red.bold(`Failed (${failed.length}):`))
            console.log(failed.map(s => s.e.collection))
            if (verbose) {
              failed.map(s => {
                const { error, collection } = s.e;
                console.log("---------------------------------------------------------------------------------------------")
    ...
    runCollectionAsync(collection, environment){
