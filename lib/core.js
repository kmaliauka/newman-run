...
class NewmanConfig{
    ...
    async looprun(root_json_file, series = false, verbose = false){
        ...
        if (series) {
          ...
          let results = []
          Promise.each(run_list, data => reflect(this.runCollectionAsync(data.collection, data.environment, data.sslClientCert, data.sslClientKey)).then(res => results.push(res)))
          .then(() => {
